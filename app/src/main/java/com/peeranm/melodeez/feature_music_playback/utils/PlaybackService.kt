...
class PlaybackService : MediaBrowserServiceCompat(),
    ...
    private suspend fun getTracksOfAlbum(albumId: Long): List<Track> {
        val ( _, tracks) = albumUseCases.getAlbumWithTracks(albumId) ?: throw Exception("No tracks found")
        return tracks
    }

    private suspend fun getTracksOfPlaylist(playlistId: Long): List<Track> {
        val ( _, tracks) = playlistUseCases.getPlaylistWithTracks(playlistId) ?: throw Exception("No tracks found")
        return tracks
    }
